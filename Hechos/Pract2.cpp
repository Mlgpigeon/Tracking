...
int main(int argc, char **argv)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //                Fin TAREA 2 y 3
        //***************************************************
        cv::Mat Rot = cv::Mat(1,3, CV_32F);
        cv::Mat camera_rot_matrix;
        glm::mat3 glm_rot_matrix;
        glm::mat3 glm_rot_matrix2;
        glm::mat4 new_view;
        glm::vec3 look_pos;
        glm::mat3 base_rot_matrix = glm::mat3(1);

        base_rot_matrix[0][0] = 0.0;
        base_rot_matrix[0][1] = 0.0;
        base_rot_matrix[0][2] = 1.0;
        base_rot_matrix[1][0] = 1.0;
        base_rot_matrix[1][1] = 0.0;
        base_rot_matrix[1][2] = 0.0;
        base_rot_matrix[2][0] = 0.0;
        base_rot_matrix[2][1] = 1.0;
        base_rot_matrix[2][2] = 0.0;

        glm::mat3 base_inverse = glm::inverse(base_rot_matrix);
        bool fake;
        ...
        if (TheMarkers.size() > 0)
        {
            for (unsigned int i = 0; i < TheMarkers.size(); i++)
            {
                ...
                Rot.at<float>(0, 0) = TheMarkers[i].Rvec.at<float>(0, 0);
                Rot.at<float>(0, 1) = -TheMarkers[i].Rvec.at<float>(0, 1);
                Rot.at<float>(0, 2) = TheMarkers[i].Rvec.at<float>(0, 2);
                
                //TheMarkers[i].calculateExtrinsics(TheMarkerSize, TheCameraParameters);
                current_pos = glm::vec3(TheMarkers[i].Tvec.at<float>(0, 0), -TheMarkers[i].Tvec.at<float>(0, 1), -TheMarkers[i].Tvec.at<float>(0, 2));
                
                cv::Rodrigues(Rot, camera_rot_matrix);
                fake = fromCV2GLM_3x3(camera_rot_matrix, &glm_rot_matrix); 
                
                std::cout << "<0.0> " << glm_rot_matrix[0][0] << "<0.1> " << glm_rot_matrix[0][1] << "<0.2> " << glm_rot_matrix[0][2] << std::endl;
                std::cout << "<1.0> " << glm_rot_matrix[1][0] << "<1.1> " << glm_rot_matrix[1][1] << "<1.2> " << glm_rot_matrix[1][2] << std::endl;
                std::cout << "<2.0> " << glm_rot_matrix[2][0] << "<2.1> " << glm_rot_matrix[2][1] << "<2.2> " << glm_rot_matrix[2][2] << std::endl;
                std::cout << " " << std::endl;
                ...
                look_vector = base_inverse * glm_rot_matrix * look_vector;
                look_vector = look_vector + current_pos;


                up_vector = glm::vec3(0.0, 1.0, 0.0);
                up_vector = base_inverse * glm_rot_matrix * up_vector;
            }
        }     
        ...
        int i = 0;
        //glm::vec3 desp = glm::cross(look_vector, up_vector);
        //desp = glm::normalize(desp);

        float EyeSeparation = 0.0065f;
        float EyeMiddle = EyeSeparation / 2.f;
        float focus_distance = 0.2f;
        float desp_factor = atan2(EyeMiddle, focus_distance);
        auto leftOffset = glm::rotate(desp_factor, glm::vec3(0, -1, 0))
            * glm::translate(glm::vec3(-EyeMiddle, 0, 0));
        auto rightOffset = glm::rotate(desp_factor, glm::vec3(0, 1, 0))
            * glm::translate(glm::vec3(EyeMiddle, 0, 0));
